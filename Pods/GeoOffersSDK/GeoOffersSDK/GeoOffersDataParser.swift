...
public class GeoOffersPushNotificationProcessor {
    ...
    public func shouldProcessRemoteNotification(_ notification: [String: AnyObject]) -> Bool {
        let aps = notification["aps"] as? [String: AnyObject] ?? [:]
        return aps["content-available"] as? String == "1"
    }

    public func parseNearbyFences(jsonData: Data) -> Int? {
        ...
        if let data = data {
            listingCache.replaceCache(data)
        }
        return data?.clientID
    }

    public func handleNotification(_ notification: [String: AnyObject]) -> Bool {
